...
#ifdef TEST_POLL
#define pl_realloc (void *)realloc
#define pl_malloc (void *)malloc
void _pl_u_fdwatch(int fd, int on) {}
#endif
...
void
_pl_u_event_src(int fd, void (*callback)(void *), void *context)
{
  if (callback) {
    if (poll_nfds>=poll_maxfd) {
      unsigned long nn = poll_maxfd + 4;
      poll_fds = pl_realloc(poll_fds, sizeof(struct pollfd)*nn);
      poll_handler = pl_realloc(poll_handler, sizeof(u_handler)*nn);
      poll_maxfd = nn;
    }
...
void
_pl_u_prepoll(int (*conditional)(void *), void *context)
{
  if (conditional) {
    if (prepoll_n>=prepoll_mx) {
      prepoll = pl_realloc(prepoll, sizeof(u_prehandler)*(prepoll_mx+4));
      prepoll_mx += 4;
    }
...
_pl_u_poll(int timeout)
{
  ...
  if (!poll_fds) poll_fds = pl_malloc(sizeof(struct pollfd));
...
int
_pl_u_poll1(int fd, int timeout)
{
  int n;
  struct pollfd pfd;
  pfd.fd = fd;
