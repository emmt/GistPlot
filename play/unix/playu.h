/*
 * $Id: playu.h,v 1.1 2005-09-18 22:05:40 dhmunro Exp $
 * UNIX-private portability layer declarations
 */
/* Copyright (c) 2005, The Regents of the University of California.
 * All rights reserved.
 * This file is part of yorick (http://yorick.sourceforge.net).
 * Read the accompanying LICENSE file for details.
 */

#include <play/extern.h>
PL_BEGIN_EXTERN_C

extern void (*u_on_idle)(void);

/* global objects used by main() for basic program flow control
 * and ANSI C main() semantics */
extern int _pl_u_main_loop(int (*on_launch)(int,char**), int, char **);
extern int _pl_u_waiter(int wait);
extern void (*_pl_u_exception)(int signal, char *errmsg);
extern char *_pl_u_errmsg;

/* if set non-zero, pl_abort will call this */
extern void (*_pl_u_abort_hook)(void);

extern void _pl_u_fpu_setup(int when);

/* arrange to call callback(context) when input or error arrives on fd
 * - callback==0 cancels */
extern void _pl_u_event_src(int fd, void (*callback)(void *), void *context);
extern void _pl_u_prepoll(int (*conditional)(void *), void *context);

/* be sure a child does not have access to known open files */
extern void _pl_u_closeall(void);
extern void _pl_u_fdwatch(int fd, int on);

/* wait at most timeout milliseconds for input on any descriptor
 * for which _pl_u_event_src has been called; timeout==-1 means forever
 * - returns 0 on timeout or signal, 1 if callback made, <0 if error
 * - at most one callback made per call to _pl_u_poll */
extern int _pl_u_poll(int timeout);

/* wait at most timeout milliseconds for input to arrive on fd only
 * - returns 1 if input arrived, 0 if timeout or signal, <0 if error
 * - no callback is made, you have to read fd after this if >0 */
extern int _pl_u_poll1(int fd, int timeout);

/* check whether this program is running in background */
extern int _pl_u_in_background(void);

/* expand ~ and $ENV_VAR in pathnames
 * - return value is in pl_wkspc, be careful not to clobber */
extern char *_pl_u_pathname(const char *pathname);

/* find argv[0] on PATH, track symbolic links to actual dir entry */
extern char *_pl_u_find_exe(const char *argv0);
extern char *_pl_u_track_link(const char *name);

PL_END_EXTERN_C
