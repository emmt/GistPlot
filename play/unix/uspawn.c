...
struct _pl_spawn {
  ...
  void *ctx;
  pl_spawn_t *next;  /* u_chldlist of all child processes */
};

#ifndef NO_PROCS
static pl_spawn_t *u_chldlist = 0;

static int u_prechld(void *dummy);
static int u_setpre = 0;

static void u_spawf0(pl_spawn_t *proc, int nocallback);
...
#  define O_NONBLOCK O_NDELAY
  ...
  pid = fork();
  if (pid) {
    /* parent, program that called pl_spawn */
    pl_spawn_t *proc = pl_malloc(sizeof(pl_spawn_t));
    ...
    if (!u_setpre) _pl_u_prepoll(u_prechld, &u_setpre), u_setpre = 1;
    _pl_u_event_src(proc->fdout, u_callout, proc);
    if (err) _pl_u_event_src(proc->fderr, u_callerr, proc);
    return proc;
...
#ifndef NO_PROCS
static void
u_spawf0(pl_spawn_t *proc, int nocallback)
{
  if (proc) {
    pl_spawn_t *list = u_chldlist;
    int in=proc->fdin, out=proc->fdout, err=proc->fderr;

    if (err>=0) _pl_u_event_src(err, 0, proc);
    if (out>=0) _pl_u_event_src(out, 0, proc);
    if (proc->pid) {
      int status;
...
u_prechld(void *dummy)
{
  ...
  if (!u_chldlist) {
    u_setpre = 0;
    _pl_u_prepoll(0, &u_setpre);  /* note that serviced == 1 always */
  }
