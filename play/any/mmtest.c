/*
 * $Id: mmtest.c,v 1.1 2005-09-18 22:05:42 dhmunro Exp $
 * test play/std.h memory manager
 */
...
main(int argc, char *argv[])
{
  ...
  if (count>0) pl_mminit();
  else count = -count;
  pl_mmfail = &mmfail;

  ptrs = pl_malloc(sizeof(void *)*n);

  /* first malloc all n and set their values */
  for (i=0 ; i<n ; i++) {
    len = le_random(mx-5) + 6;
    ptrs[i] = pl_malloc(len);
