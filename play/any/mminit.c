...
long pl_mmoffset = MM_OFFSET*sizeof(mm_block);
long pl_mmextra = MM_EXTRA*sizeof(mm_block);
#endif
...
expand_arena(mm_arena *arena)
{
  ...
  if (!block) return pl_mmfail(size);
...
brealloc(void *p, size_t n)
{
  if (p) {
    ...
    if (arena) {
      ...
      new_block = bmalloc(n);
      if (!new_block) return pl_mmfail(n<=0? 1 : n);
