...
con_stdinit(void (**wout)(char*,long), void (**werr)(char*,long))
{
  ...
  *werr = con_stderr;
   ...
   * - in any event, just using w_hin to _pl_w_add_input fails in
  ...
  if (stdin_ready && stdin_accepted &&
    ...
    if (h) {
      _pl_w_add_input(stdin_ready, con_sender, 0);
      Sleep(0);
      CloseHandle(h);
    } else {
