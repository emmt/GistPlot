...
{
  HMODULE hm = me;
  HWND hw = CreateWindow(_pl_w_win_class, "dummy main wnd", 0,
                        CW_USEDEFAULT, 0, 1, 1, 0, 0, hm, 0);

  _pl_w_initialize(hm, hw, cyg_quit, 0, 0);
  ...
  w_get_cmds(cmd_line);
  _pl_w_protect(w_loop);
  if (w_result) return w_result;

  return _pl_w_on_quit();
}
...
w_get_cmds(LPSTR cmd_line)
{
  ...
  DWORD len = GetModuleFileName(_pl_w_app_instance, module_name, 1024);
  module_name[len] = '\0';
  w_argc = 0;
  w_argv = (char**)HeapAlloc(heap, HEAP_GENERATE_EXCEPTIONS, sizeof(char *)*9);
  w_argv[w_argc++] = w_str_cpy(heap, _pl_w_unixpath(module_name), len);
  if (cmd_line) {
    char *c = cmd_line;
    char delim;
    for (;;) {
...
{ /* this function runs in its own thread */
  return (type==CTRL_C_EVENT)? _pl_w_sigint(1) : FALSE;
}
