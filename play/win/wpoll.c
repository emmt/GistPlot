...
DWORD _pl_w_id_worker = 0;
HANDLE _pl_w_worker = 0;
...
void
_pl_w_initialize(HINSTANCE i, HWND w, void (*wquit)(void),
             int (*wstdinit)(void(**)(char*,long), void(**)(char*,long)),
             HWND (*wparent)(int, int, char *, int))
{
  _pl_w_app_instance = i;
  _pl_w_main_window = w;
  w_quit = wquit;
  _pl_w_stdinit = wstdinit;
  _pl_w_parent = wparent;
}
...
pl_qclear(void)
{
  /* causes _pl_w_app_msg (the PreTranslate method) to clear worker queue */
  w_qclearing = 1;
}

int
_pl_w_work_idle(void)
{
  if (w_poll(0) == WAIT_TIMEOUT) {
    double dtime = pl_timeout();
    UINT timeout = (dtime<0. || dtime>2.e9)? INFINITE : (UINT)(1000.*dtime);
    if ((w_nputs+_pl_w_nwins) || timeout!=INFINITE) {
...
_pl_w_pollinit(void)
{
  if (!w_inputs) {
    ...
    w_msg_callbacks = HeapAlloc(heap, HEAP_GENERATE_EXCEPTIONS,
                               sizeof(void (*)())*8);
    _pl_w_id_worker = GetCurrentThreadId();
    DuplicateHandle(GetCurrentProcess(), GetCurrentThread(),
                    GetCurrentProcess(), &_pl_w_worker, 0, FALSE,
...
void
_pl_w_prepoll(void (*on_prepoll)(void *), void *context, int remove)
{
  if (remove) {
    struct w_prepoll_item *pre = w_pre_items;
