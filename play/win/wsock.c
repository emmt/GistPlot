...
struct _pl_sckt {
  SOCKET fd;
  char *peer;
  pl_sckt_cb_t *callback;  /* int pl_sckt_cb_t(pl_sckt_t *sock, void *ctx); */
  void *ctx;
...
static pl_sckt_t *psckt_list = 0;  /* for on_quit */

static pl_sckt_t *psckt_setup(pl_sckt_t *sock, void *ctx, pl_sckt_cb_t *callback);
static void psckt_stop_thread(pl_sckt_t *sock);
static int psckt_shutdown(pl_sckt_t *sock);
...
static pl_sckt_t *
psckt_setup(pl_sckt_t *sock, void *ctx, pl_sckt_cb_t *callback)
{
  ...
  if (callback) {
     ...
     */
    _pl_w_add_input(sock->ready, psckt_cb_manager, sock);
    _pl_w_prepoll(psckt_prepoller, sock, 0);
  }
  return sock;
}

static void
psckt_stop_thread(pl_sckt_t *sock)
{
  sock->finished = 1;
  if (sock->ready != INVALID_HANDLE_VALUE) {
    /* prevent yorick thread (this thread) from waiting for this socket */
    _pl_w_add_input(sock->ready, 0, sock);
    _pl_w_prepoll(psckt_prepoller, sock, 1);
    sock->callback = 0;
    sock->ctx = 0;
  }
...
pl_sckt_t *
pl_sckt_listen(int *pport, void *ctx, pl_sckt_cb_t *callback)
{
  SOCKET lfd;
  int port = *pport;
  pl_sckt_t *listener = pl_malloc(sizeof(pl_sckt_t));
  struct addrinfo *ai, *ailist = listener? psckt_get_ailist(0, port, 1) : 0;
  if (!ailist) {
    if (listener) pl_free(listener);
    return 0;
  }
...
pl_sckt_t *
pl_sckt_accept(pl_sckt_t *listener, char **ppeer, void *ctx, pl_sckt_cb_t *callback)
{
  struct sockaddr_storage sad;
  socklen_t lsad = sizeof(struct sockaddr_storage);
  struct sockaddr *psad = (struct sockaddr *)&sad;
  char host[NI_MAXHOST];
  pl_sckt_t *sock = pl_malloc(sizeof(pl_sckt_t));
  SOCKET sfd, lfd = sock->fd;

  *ppeer = 0;
  if (listener->peer || !sock) {  /* listeners have no peer */
    if (sock) pl_free(sock);
    return 0;
  }
  ...
  sock->fd = sfd;
  sock->peer = *ppeer = pl_strcpy(host);
  return psckt_setup(sock, ctx, callback);
}

pl_sckt_t *
pl_sckt_connect(const char *addr, int port, void *ctx, pl_sckt_cb_t *callback)
{
  SOCKET sfd;
  pl_sckt_t *sock = pl_malloc(sizeof(pl_sckt_t));
  ...
  sock->fd = sfd;
  sock->peer = pl_strcpy(addr);
  return psckt_setup(sock, ctx, callback);
}

/* return value <0 for error, <len means socket recv side closed */
long
pl_sckt_recv(pl_sckt_t *sock, void *msg, long len)
{
  char *cmsg = msg;
  int n;
...
int
pl_sckt_send(pl_sckt_t *sock, const void *msg, long len)
{
  const char *cmsg = msg;
  int n;
  while (len > 0) {
...
pl_sckt_t *
pl_sckt_accept(pl_sckt_t *sock, char **peer, void *ctx, pl_sckt_cb_t *callback)
{
  return 0;
}
pl_sckt_t *
