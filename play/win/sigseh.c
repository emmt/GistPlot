...
_pl_w_protect(int (*run)(void))
{
  ...
  do {
    __try {
      if (pl_signalling) _pl_w_caught(), w_catch_count = 0;
      result = run();
      w_quitting = 1;
...
w_catch(int code)
{
  if (!code && pl_signalling)                      code = pl_signalling;
  else if (!code && !pl_signalling)                code = PL_SIG_SOFT;
  ...
  if (code==PL_SIG_FPE) _pl_w_fpu_setup();
