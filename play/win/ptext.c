  ...
  if (dc) {
    ...
    if (!w->orient) {
      ...
      if (n>0 && GetTextMetrics(dc, &tm) &&
        ...
        void *monodat = pl_malloc(scandx*scandy/4);
        ...
        if (monodat) {
          COLORREF bg = _pl_w_color(w,w->bg);
          COLORREF fg = _pl_w_color(w,w->color);
          if (w->orient == 2) {
            pl_mrot180(monodat, monorot, scandx, height);
            x0 -= width-1;
            y0 -= (height-base)-1;
            scandx = width, width = height, height = scandx;
          } else if (w->orient == 1) {
            pl_mrot090(monodat, monorot, scandx, scandy);
            x0 -= base;
            y0 -= width-1;
          } else {
            pl_mrot270(monodat, monorot, scandx, scandy);
            x0 -= (height-base)-1;
          }
