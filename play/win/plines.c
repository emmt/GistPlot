/*
 * $Id: plines.c,v 1.1 2005-09-18 22:05:36 dhmunro Exp $
 * pl_lines for MS Windows
 */
...
pl_lines(pl_win_t *w)
{
  ...
  HDC dc = _pl_w_getdc(w, 2);
  if (dc && n>0) {
    POINT *pxy = _pl_w_pt_list;
    if (n>1) {
      int closed = (pxy[0].x==pxy[n-1].x && pxy[0].y==pxy[n-1].y);
      if ((w->pen_type&7)==PL_SOLID || w->s->does_linetypes) {
...
void
pl_segments(pl_win_t *w)
{
  int n = _pl_w_pt_count / 2;
  HDC dc = _pl_w_getdc(w, 2);
  if (dc && n>0) {
    POINT *pxy = _pl_w_pt_list;
    while (n-- > 0) {
...
pl_dots(pl_win_t *w)
{
  ...
  if (dc) {
    POINT *pxy = _pl_w_pt_list;
    COLORREF color = _pl_w_color(w, w->color);
    while (n-- > 0) {
