...
void
pl_palette(pl_win_t *w, unsigned long *colors, int n)
{
  pl_scr_t *s = w->s;
  int i;
  unsigned int r, g, b;
   ...
   * window palette at creation time, never call pl_palette */
  ...
  if (pl_signalling == PL_SIG_NONE) {
    for (i=0 ; i<n ; i++) {
      ...
      } else {  /* must be consistent with _pl_w_color in getdc.c */
        r = (r+32)>>6;
        g = (g+16)>>5;
        b = (b+32)>>6;
        g += b+(b<<3);
    ...
    for (; i<w->n_pixels ; i++)
      w->pixels[i] = s->sys_colors[255-PL_FG];
    w->n_pixels = n;
      ...
      if (n) {
        ...
        if (w->palette) {
          if (s->active==w || !s->active) {
             ...
             * pl_palette before a redraw display list sequence */
          }
