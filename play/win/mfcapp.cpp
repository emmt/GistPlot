...
mfc_frame_wnd::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  if (message==ID_CALL_FUNC) {
    ...
    if (message==WM_QUERYNEWPALETTE && _pl_w_screen.active)
      return ::SendMessage(_pl_w_screen.active->w, message, wParam, lParam);
...
{
  USES_CONVERSION;
  _pl_w_initialize(the_boss.m_hInstance, the_boss.m_pMainWnd->m_hWnd,
  ...
  w_argv[w_argc++] = mfc_cpy(heap, A2T(_pl_w_unixpath(module_name)), len);
  if (cmd_line) {
    LPTSTR c = cmd_line;
    char delim;
    for (;;) {
...
mfc_worker::Run()
{
  return _pl_w_protect(mfc_run_hack);
}
...
{
  return _pl_w_app_msg(pmsg) || CWinThread::PreTranslateMessage(pmsg);
}

BOOL
mfc_worker::OnIdle(LONG count)
{
  return CWinThread::OnIdle(count) || _pl_w_work_idle();
}

int
mfc_worker::ExitInstance()
{
  w_result = _pl_w_on_quit();
...
{
  if (child) {
    pl_win_t *pw = child? (pl_win_t *)GetWindowLong(child, GWL_USERDATA) : 0;
