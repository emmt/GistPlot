...
/* pl_popen in files.c also creates a process */
int
pl_system(const char *cmdline)
{
...
typedef struct _spawn_rd spawn_rd_t;
struct _spawn_rd {
  pl_spawn_t *parent;
...
struct _pl_spawn {
  ...
  int flags;      /* uses + 4 stderr, 8 pl_recv in callerr, 16 error */
  spawn_rd_t o[2];
};
...
pl_spawn_t *
pl_spawn(char *name, char **argv, void (*callback)(void *ctx, int err),
        void *ctx, int err)
{
  pl_spawn_t *proc;
  HANDLE ht1;
  ...
  proc = pl_malloc(sizeof(pl_spawn_t));
  if (!proc) return 0;
  ZeroMemory(proc, sizeof(pl_spawn_t));
  ...
  _pl_w_add_input(proc->o[0].ready, w_callout, &proc->o[0]);
  _pl_w_add_input(proc->o[1].ready, w_callerr, &proc->o[1]);
...
spawn_reader(LPVOID lpvThreadParam)
{
  ...
  while (chan->n >= 0 && chan->pipe) {
    ...
    do {
      /* this is a loop in case pl_recv cannot accept all chars at once */
  ...
  if (--chan->parent->flags <= 0) pl_free(chan->parent);
  return 0;
}

static void spawn_kill(pl_spawn_t *proc, int nocallback);

/* ARGSUSED */
extern void
pl_spawf(pl_spawn_t *proc, int nocallback)
{
  if (proc) {
...
static void
spawn_kill(pl_spawn_t *proc, int nocallback)
{
  if (proc) {
    if (proc->o[0].ready) _pl_w_add_input(proc->o[0].ready, 0, 0);
    if (proc->o[1].ready) _pl_w_add_input(proc->o[1].ready, 0, 0);
    SPAWN_HCLOSE(proc->in);
...
w_callout(void *vproc)
{
  ...
  pl_spawn_t *proc = chan->parent;
  if (!(proc->flags & 16)) {
    if (chan->n >= 0) {
      proc->flags &= ~8;   /* for pl_recv */
      proc->callback(proc->ctx, 0);
    } else {
      proc->flags |= 16;
      /* on error, callback will call pl_spawf with nocallback==1 */
...
int
pl_send(pl_spawn_t *proc, char *msg, long len)
{
  int result = 2;
  if (proc && proc->in) {
    result = 1;
...
long
pl_recv(pl_spawn_t *proc, char *msg, long len)
{
  if (proc && len>0) {
