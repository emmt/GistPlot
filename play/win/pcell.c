 ...
 * pl_ndx_cell, pl_rgb_cell for MS Windows
 */
...
static void w_cell(pl_win_t *w, unsigned char *ndxs, unsigned char *rgbs,
         ...
         int x0, int y0, int x1, int y1)
{
  ...
  HDC dc2 = _pl_w_getdc(offscreen, 0);
  if (dc && dc2)
    BitBlt(dc, x, y, x1-x0, y1-y0, dc2, x0, y0, SRCCOPY);
}

void
pl_rgb_read(pl_win_t *w, unsigned char *rgbs,
           int x0, int y0, int x1, int y1)
{
  HDC dc = _pl_w_getdc(w, 0);
  x1 -= x0;
...
static void
w_cell(pl_win_t *w, unsigned char *ndxs, unsigned char *rgbs,
       int ncols, int nrows, int x0, int y0, int x1, int y1)
{
  ...
  if (dc) {
    ...
    if (bmrgb) {
      ...
      if (nbits == 8) {
        ...
        for (; i<=PL_EXTRA ; i++) h.ndx[i] = 0;  /* black */
        for (; i<256 ; i++) {
