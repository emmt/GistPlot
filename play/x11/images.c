 ...
 * pl_pixmap and pl_rgbmap for X11
 */
...
static void x_image(pl_win_t *w, unsigned char *bytes, int rgb, int ncols,
                    int nrows, int x0, int y0, int x1, int y1);

static int x_put1(XImage *im, int i, int j, pl_col_t pixel);
static int x_put2b(XImage *im, int i, int j, pl_col_t pixel);
static int x_put2l(XImage *im, int i, int j, pl_col_t pixel);
static int x_put3b(XImage *im, int i, int j, pl_col_t pixel);
static int x_put3l(XImage *im, int i, int j, pl_col_t pixel);
static int x_put4b(XImage *im, int i, int j, pl_col_t pixel);
static int x_put4l(XImage *im, int i, int j, pl_col_t pixel);

void
pl_ndx_cell(pl_win_t *w, unsigned char *ndxs, int ncols, int nrows,
...
void
pl_rgb_cell(pl_win_t *w, unsigned char *rgbs, int ncols, int nrows,
           int x0, int y0, int x1, int y1)
{
...
static void
x_image(pl_win_t *w, unsigned char *bytes, int rgb, int ncols,
        int nrows, int x0, int y0, int x1, int y1)
{
  unsigned char *cell;
  ...
  if ((long)bpp*(long)width*(long)height > 288L*(long)ncols*(long)nrows) {
    ...
    _pl_x_imzap(s);  /* wasteful to have created image,
    ...
    for (ya=yb=y0 ; ya<y1 ; ya=yb) {
      ...
      for (xa=xb=x0 ; xa<x1 ; xa=xb) {
        ...
        pl_color(w, rgb? PL_RGB(cell[0], cell[1], cell[2]) : cell[0]);
    ...
    int (*put_pixel)(XImage *, int, int, pl_col_t)= 0;
    int bpl = im->bytes_per_line;
    pl_col_t pixel;

    im->data = pl_malloc(bpl*height);
    if (!im->data) { _pl_x_imzap(s); return; }
    ...
    if (put_pixel) {
      ...
      put_pixel = (int (*)(XImage *, int, int, pl_col_t))im->f.put_pixel;
      if (bpp==1) pl_color(w, PL_FG);
      y1 = height;
      bpl = 1;
      x1 = width;
      bpp = 1;
    ...
    if (ncols==width && nrows==height) {
      for (r=0 ; r<y1 ; r+=bpl)
        for (c=0 ; c<x1 ; c+=bpp) {
          pixel = rgb? PL_RGB(bytes[0], bytes[1], bytes[2]) : bytes[0];
          bytes += drow;
          put_pixel(im, c, r, _pl_x_getpixel(w, pixel));
        }
      ...
      for (ya=yb=0 ; yb<y1 ; ya=yb) {
        ...
        for (xa=xb=0 ; xb<x1 ; xa=xb) {
          ...
          pixel = rgb? PL_RGB(cell[0], cell[1], cell[2]) : cell[0];
          pixel = _pl_x_getpixel(w, pixel);
          for (j=ya ; j<yb ; j+=bpl)
            for (i=xa ; i<xb ; i+=bpp)
...
static int
x_put1(XImage *im, int i, int j, pl_col_t pixel)
{
...
static int
x_put2b(XImage *im, int i, int j, pl_col_t pixel)
{
  unsigned char *data = (unsigned char *)im->data;
  data[j+i+0] = pixel>>8;
  data[j+i+1] = pixel;
...
static int
x_put2l(XImage *im, int i, int j, pl_col_t pixel)
{
  unsigned char *data = (unsigned char *)im->data;
  data[j+i+0] = pixel;
  data[j+i+1] = pixel>>8;
...
static int
x_put3b(XImage *im, int i, int j, pl_col_t pixel)
{
  unsigned char *data = (unsigned char *)im->data;
  data[j+i+0] = pixel>>16;
  data[j+i+1] = pixel>>8;
...
static int
x_put3l(XImage *im, int i, int j, pl_col_t pixel)
{
  unsigned char *data = (unsigned char *)im->data;
  data[j+i+0] = pixel;
  data[j+i+1] = pixel>>8;
...
static int
x_put4b(XImage *im, int i, int j, pl_col_t pixel)
{
  unsigned char *data = (unsigned char *)im->data;
  data[j+i+0] = pixel>>24;
  data[j+i+1] = pixel>>16;
...
static int
x_put4l(XImage *im, int i, int j, pl_col_t pixel)
{
  unsigned char *data = (unsigned char *)im->data;
  data[j+i+0] = pixel;
  data[j+i+1] = pixel>>8;
