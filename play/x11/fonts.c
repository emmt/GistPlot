...
/* _pl_x_parse_fonts is semi-private, used only in connect.c */
extern void _pl_x_parse_fonts(pl_x_display_t *xdpy);

static int x_substitute(pl_x_display_t *xdpy, int *font, int *pixsize);
static void x_analyze(pl_x_display_t *xdpy, int fam);
...
void
_pl_x_parse_fonts(pl_x_display_t *xdpy)
{
  x_analyze(xdpy, PL_COURIER);
  x_analyze(xdpy, PL_TIMES);
  x_analyze(xdpy, PL_HELVETICA);
  x_analyze(xdpy, PL_SYMBOL);
  x_analyze(xdpy, PL_NEWCENTURY);
}
...
XFontStruct *
_pl_x_font(pl_x_display_t *xdpy, int font, int pixsize)
{
  char nm[128], *name;
  XFontStruct *f;
  if (tmp_fonts) tmp_free();

  if (font>=PL_GUI_FONT || font<0 || pixsize<=0 || pixsize>180) {
    f = xdpy->font;
    ...
    for (pass=siz=0 ;; pass++) {
      ...
      if (font==PL_GUI_FONT) return xdpy->font;
    }
...
static int
x_substitute(pl_x_display_t *xdpy, int *font, int *pixsize)
{
  int fnt = *font;
  int siz = *pixsize;
  int face = fnt&(PL_BOLD|PL_ITALIC);
  ...
  if (!xdpy->available[fnt].nsizes) {
    ...
    if (!face || !xdpy->available[fnt].nsizes) {
      ...
      if (i<4) {
        ...
        if (xdpy->available[PL_TIMES|face].nsizes)
          fnt = PL_TIMES|face;
        else if (xdpy->available[PL_NEWCENTURY|face].nsizes)
          fnt = PL_NEWCENTURY|face;
        else if (xdpy->available[PL_HELVETICA|face].nsizes)
          fnt = PL_HELVETICA|face;
        else if (xdpy->available[PL_COURIER|face].nsizes)
          fnt = PL_COURIER|face;
        else if (face) {
          if (xdpy->available[PL_TIMES].nsizes)
            fnt = PL_TIMES;
          else if (xdpy->available[PL_NEWCENTURY].nsizes)
            fnt = PL_NEWCENTURY;
          else if (xdpy->available[PL_HELVETICA].nsizes)
            fnt = PL_HELVETICA;
          else if (xdpy->available[PL_COURIER].nsizes)
            fnt = PL_COURIER;
          else
            fnt = PL_GUI_FONT;
        } else
...
static void
x_analyze(pl_x_display_t *xdpy, int fam)
{
  ...
  for (i=0 ; i<n ; i++) {
    ...
    if (nsizes%12==0) {
      ...
      xdpy->available[face].sizes = pl_realloc(sizes, sizeof(int)*(nsizes+12));
      if (!xdpy->available[face].sizes) {
        xdpy->available[face].sizes = sizes;
        return;
      }
      xdpy->available[face].names = pl_realloc(names,sizeof(char*)*(nsizes+13));
    ...
    if (pixsize) {
      xdpy->available[face].names[j] = pl_strcpy(tmp_fonts[i]);
    } else {
      ...
      *(pnm++)= '*';   *(pnm++)= '-';
      ...
      while ((*(pnm++)= *(name++)));
      xdpy->available[face].names[j] = pl_strcpy(nm);
    }
  }
