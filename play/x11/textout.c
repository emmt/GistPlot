...
{
  XFontStruct *f = _pl_x_font(s->xdpy, font, pixsize);
...
pl_font(pl_win_t *w, int font, int pixsize, int orient)
{
  ...
  if (s->rotgc || s->tmp || s->image || s->pixmap!=None) _pl_x_rotzap(s);
  ...
  if (!orient) {
    ...
    /* defer rotated font selection to pl_text call */
    s->rotgc_font = font;
...
void
pl_text(pl_win_t *w, int x0, int y0, const char *text, int n)
{
  pl_scr_t *s = w->s;
  pl_x_display_t *xdpy = s->xdpy;
  Display *dpy = xdpy->dpy;
  int orient = s->rotgc_orient;
  GC gc = _pl_x_getgc(s, w, orient? FillStippled : FillSolid);
  Drawable d = w->d;
  int i;
  ...
  if (!orient) {
    ...
    XFontStruct *f = _pl_x_font(xdpy, s->rotgc_font, s->rotgc_pixsize);
    int width = XTextWidth(f, (char *)text, n);
    int height = f->ascent + f->descent;
    XGCValues values;

    char *data = pl_malloc(((width-1)/8+1)*height);
    if (!data) { _pl_x_rotzap(s);  return; }
    ...
    if (orient==2) {
      s->image->data = pl_malloc(((width-1)/8+1)*height);
      if (!s->image->data) { _pl_x_rotzap(s);  return; }
      s->pixmap = XCreatePixmap(dpy, s->root, width, height, 1);
      pl_lrot180((void *)data, (void *)s->image->data, width, height);
      ...
      _pl_x_imzap(s);
      s->image = XCreateImage(dpy, (Visual *)0, 1, XYBitmap, 0,
                              pl_malloc(((height-1)/8+1)*width),
                              height, width, 8, 0);
      if (!s->image->data) { _pl_x_rotzap(s);  return; }
      s->image->byte_order = s->image->bitmap_bit_order = LSBFirst;
      s->pixmap = XCreatePixmap(dpy, s->root, height, width, 1);
      if (orient==1) {
        pl_lrot090((void *)data, (void *)s->image->data, width, height);
        x0 -= f->ascent;
        y0 -= width-1;
      } else {
        pl_lrot270((void *)data, (void *)s->image->data, width, height);
        x0 -= f->descent-1;
      }
      XPutImage(dpy, s->pixmap, s->rotgc, s->image, 0,0, 0,0, height,width);
    }

    /* stipple fillstyle already set by _pl_x_getgc,
