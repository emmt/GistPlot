...
/* WC->NDC mapping stored by ga_draw_ticks, used by Draw?Ticks
   and Draw?Labels routines */
static gp_real_t scalx, offx, scaly, offy;

/* The main output from FindTicks is a cumulative count of the
   number of ticks at each level, nChangeLevel[0:nLevel]
   (nLevel<GA_TICK_LEVELS), and the floating point tick values
...
static ga_ticker_t *altticks= 0;
static ga_labeler_t *altlabel= 0;
...
static void EvenlySpace(gp_real_t start, gp_real_t unit, gp_real_t stop);
static gp_real_t Subdivide(gp_real_t unit, int nDivs, gp_real_t lo, gp_real_t hi,
                        gp_real_t *start);
static void FirstSubDecade(gp_real_t *unit, gp_real_t lo, gp_real_t hi,
                           gp_real_t zSmallest);
static int LogDivide(gp_real_t *unit, int *base, gp_real_t *fine,
                     gp_real_t lo, gp_real_t hi, gp_real_t zSmallest);
static void ScopeOutTicks(gp_real_t *loin, gp_real_t *hiin,
                          ga_axis_style_t *style, int isLog,
                          int *useLog, gp_real_t *nMajor, gp_real_t *nMinor,
                          gp_real_t *jUnit, int *jBase, int *jPower,
                          gp_real_t *itick0);
static int FindOmitted(gp_real_t lo, gp_real_t hi);
static gp_real_t FindOrigin(gp_real_t lo, gp_real_t hi, ga_axis_style_t *style, int isLog);
static void FindTicks(gp_real_t lo, gp_real_t hi, ga_axis_style_t *style, int isLog);
static void TickEndpoints(gp_real_t place, gp_real_t len, int lo, int hi,
                          gp_real_t *p0, gp_real_t *p1);
static void DrawXTicks(gp_real_t place, gp_real_t *lens, int inout, int upper,
                       gp_line_attribs_t *attribs);
static void DrawYTicks(gp_real_t place, gp_real_t *lens, int inout, int upper,
                       gp_line_attribs_t *attribs);
static void DrawOrigin(gp_real_t x0, gp_real_t x1, gp_real_t y0, gp_real_t y1,
                       gp_line_attribs_t *attribs);
static void InitLabels(int nDigits, char exponent[]);
static void NextLabel(char label[]);
static void NiceLogLabel(char *label, char *format, gp_real_t value, gp_real_t tick);
static void GtText(gp_real_t x0, gp_real_t y0, const char *text);
static int DrawXLabels(int isLog, gp_real_t place, int upper, int nDigits,
                       gp_text_attribs_t *attribs);
static int DrawYLabels(int isLog, gp_real_t place, int upper, int nDigits,
                       gp_text_attribs_t *attribs);
static void DrawOverflow(gp_real_t x, gp_real_t y);
...
{
  if (start<=stop) {
    gp_real_t v, vo;  /* avoid infinite loop if stop-start and unit  << start */
...
static gp_real_t Subdivide(gp_real_t unit, int nDivs, gp_real_t lo, gp_real_t hi,
                        gp_real_t *start)
{
  gp_real_t newUnit= unit/(gp_real_t)nDivs;
  gp_real_t value= *start;
  gp_real_t test= lo+unit;
  int j;
  for (j=1 ; j<nDivs ; j++) {
    value+= newUnit;
    if (value>=test) {
...
static gp_real_t zi[3]= {1.0, 2.0, 5.0};
static gp_real_t zf[3]= {2.0, 5.0, 10.0};
static gp_real_t zlogi[3]= {0.0, LOG2, LOG5};
static gp_real_t zlogf[3]= {LOG2, LOG5, 1.0};

#define EPS_LOG 0.00001

static gp_real_t zfSigh[3]= {2.0+EPS_LOG, 5.0+EPS_LOG, 10.0-EPS_LOG};

static void FirstSubDecade(gp_real_t *unit, gp_real_t lo, gp_real_t hi,
                           gp_real_t zSmallest)
{
  int ii, iiset= -1;
  gp_real_t z, zlog, oldUnit;
  gp_real_t decade= ceil(lo);
  gp_real_t zDecTest= lo+1.0;
  gp_real_t interval0or1;
                     ...
                     gp_real_t lo, gp_real_t hi, gp_real_t zSmallest)
{
  ...
  gp_real_t z, zu, zlog, ztop, newUnit, oldUnit;
  gp_real_t du, decade= ceil(lo);
  gp_real_t zDecTest= lo+1.0;
  gp_real_t zTest;
  int anyDivisions= 0;
...
static gp_real_t nMajor;     /* extracted from GaAxis style, adjusted if... */
static gp_real_t nMinor;     /* ...this is log scale reverted to linear */
static gp_real_t jUnit;      /* nice number just above delta/nMajor */
static int jBase, jPower; /* jUnit= jBase*10^jPower, jBase= 1, 2, or 5 */
static gp_real_t itick0;     /* integer "number of major unit" for 1st tick */

/* After FindTicks, first tick is at tick0, and first major tick
   has index itick0 (tick is at itick0*jUnit) */
static gp_real_t tick0;
                          ...
                          gp_real_t *itick0)
{
  gp_real_t lo= *loin;
  gp_real_t hi= *hiin;
  gp_real_t finest, delta;

  *nMajor= style->nMajor;
  *nMinor= style->nMinor;
  if (*nMinor<*nMajor) *nMinor= *nMajor;

  if (hi<lo) { gp_real_t tmp= lo; lo= hi; hi= tmp; }
  delta= hi-lo;
  if (delta==0.0) {
    if (lo>0.0) delta= lo;
...
static int FindOmitted(gp_real_t lo, gp_real_t hi)
{
  gp_real_t z= itick0*jUnit;
  int nTicks= nChangeLevel[0];
  if (nTicks>0) {
    gp_real_t multiple= 100./(gp_real_t)jBase; /* multiple of jUnit ends in "00" */
    multiple*= ceil(itick0/multiple);    /* itick0 rounded up to multiple */
    nTicks--;
    if (multiple <= itick0+(gp_real_t)nTicks) return (int)(multiple-itick0);
    else return 0;
  } else {
...
{
  int useLog;
  gp_real_t nMajor, nMinor, jUnit;
  int jBase, jPower;
  gp_real_t itick0;

  gp_real_t z, multiple, origin;
  int nTicks;
  ...
  if (nTicks>0) {
    multiple= 100./(gp_real_t)jBase; /* multiple of jUnit that ends in "00" */
    multiple*= ceil(itick0/multiple);   /* itick0 rounded up to multiple */
    nTicks--;
    if (multiple <= itick0+(gp_real_t)nTicks) origin= multiple*jUnit;
...
static void FindTicks(gp_real_t lo, gp_real_t hi, ga_axis_style_t *style, int isLog)
{
  gp_real_t finest, finest2, finest5, finest10;
  gp_real_t nUnit = 0;
  int /*nBase,*/ nPower;
  int reqdSpace;
  ...
  if (altticks && !isLog) {
    int i;
    for (i=0 ; i<GA_TICK_LEVELS ; i++) nChangeLevel[i]= 0;
    if (!altticks(lo, hi, nMajor, nMinor, ticks, nChangeLevel)) {
      nTotal= nChangeLevel[0];
      for (nLevel=GA_TICK_LEVELS-1 ; nLevel>0 ; nLevel--)
  ...
  for (;;) {  /* Loop to finer and finer decades */
    /* nUnit is 1.0eNN */
    if (nUnit<finest2 || (useLog&&nPower==0) || nLevel>=GA_TICK_LEVELS-1) {
      /*nBase= 1;*/
      break;
    }
    nPower--;
  ...
  mid:
    /* nUnit is 5.0eNN */
    if (nUnit<finest5 || nLevel>=GA_TICK_LEVELS-1) {
      /*nBase= 5;*/
      break;
    }
    nUnit= Subdivide(nUnit, 5, lo, hi, &tick0);
  ...
  subDecadeTicks= 0;
  if (useLog && nUnit<1.1 && nLevel<GA_TICK_LEVELS-1) {
    gp_real_t delta= hi-lo;
    gp_real_t logAdjMajor= style->logAdjMajor;
    gp_real_t logAdjMinor= style->logAdjMinor;
    gp_real_t fine[3];  /* 0 is interval (1,2], 1 is (2,5], 2 is (5,10) */
    gp_real_t unit[3];
    int base[3];
    gp_real_t zSmallest;
                       ...
                       gp_line_attribs_t *attribs)
{
  int i, level, llev;
  gp_real_t y0, y1;
  int lo= upper? (inout & GA_TICK_IN) : (inout & GA_TICK_OUT);
  int hi= upper? (inout & GA_TICK_OUT) : (inout & GA_TICK_IN);
  ...
  ga_attributes.l= *attribs;
  gp_draw_disjoint(nTotal, px, py, qx, qy);
}
...
{
  ga_attributes.l= *attribs;
  gp_draw_disjoint(1L, &x0, &y0, &x1, &y1);
}
...
static gp_real_t iValue;  /* integer value of current label */
static int omitX, omitY;  /* indices of origin label for GA_TICK_C */
...
static void InitLabels(int nDigits, char exponent[])
{
  ...
  if (useLog) {
    ...
    int subDigits= (int)minSubdecade + subPower[0];
    int jp= jPower;
    gp_real_t fValue= iValue + (gp_real_t)((nLabels-1)*jBase);
    ...
    gp_real_t origin= 100.0*ceil(iValue/100.0); /* round iValue up to 100s */
    sprintf(fixedFormat, "%%+0%d.0f", 2+iPower);
    overflowChar= iPower;   /* first iPower-1 digits are overflow, last
                               two are used on individual labels */
    if (origin-iValue > (gp_real_t)(jBase*(nLabels-1))) origin= iValue;
...
static gp_real_t positiveZero= 0.0; /* Yecch */
...
static void NextLabel(char label[])
{
  ...
  if (decimalPoint) {
    ...
    /* E-12 style decade */
    sprintf(label, gp_do_escapes? nicedFormat:decadeFormat, iValue);
  } else {
                       ...
                       gp_text_attribs_t *attribs)
{
  ...
  ga_attributes.t= *attribs;
  ga_attributes.t.alignH= GP_HORIZ_ALIGN_CENTER;
  ga_attributes.t.alignV= upper? GP_VERT_ALIGN_BASE : GP_VERT_ALIGN_CAP;

  exponent= expspace+2;
  InitLabels(nDigits, exponent); /* also sets overflow */
  if (gp_do_escapes && exponent[0]) {
    exponent= expspace;
    exponent[0]= '1';
    exponent[1]= '0';
    exponent[2]= '^';
  ...
  if (exponent[0]) {
    ...
    gp_real_t exploc;
    gp_real_t nudged= place + (upper? +ga_attributes.t.height : -ga_attributes.t.height);
    if (nLabel>1) {
      exploc= 0.5*(ticks[nLabel-1]+ticks[nLabel-2]);
      exploc= scalx*exploc+offx;
    } else {
      if (fabs(tick0-gp_transform.viewport.xmin) >
          fabs(tick0-gp_transform.viewport.xmax))
        exploc= 0.5*(tick0+gp_transform.viewport.xmin);
      else
        exploc= 0.5*(tick0+gp_transform.viewport.xmax);
    }
    gp_draw_text(exploc, nudged, exponent);
  }
                       ...
                       gp_text_attribs_t *attribs)
{
  ...
  ga_attributes.t= *attribs;
  ga_attributes.t.alignH= upper? GP_HORIZ_ALIGN_LEFT : GP_HORIZ_ALIGN_RIGHT;
  ...
  if (exponent[0]) {
    ...
    gp_real_t exploc;
    gp_real_t nudged= place + (upper? +0.4 : -0.4)*ga_attributes.t.height;
    if (nLabel>1) {
      exploc= 0.5*(ticks[nLabel-1]+ticks[nLabel-2]);
      exploc= scaly*exploc+offy;
    } else {
      if (fabs(tick0-gp_transform.viewport.ymin) >
          fabs(tick0-gp_transform.viewport.ymax))
        exploc= 0.5*(tick0+gp_transform.viewport.ymin);
      else
        exploc= 0.5*(tick0+gp_transform.viewport.ymax);
    }
    gp_draw_text(nudged, exploc, exponent);
  }
...
static void DrawOverflow(gp_real_t x, gp_real_t y)
{
  ...
  gp_draw_text(x, y, overflow);
}

/* ------------------------------------------------------------------------ */

static gp_transform_t saveMap;
static gp_transform_t unitMap= {{0., 2., 0., 2.}, {0., 2., 0., 2.}};

int ga_draw_ticks(ga_tick_style_t *ticks, int xIsLog, int yIsLog)
{
  return ga_draw_alt_ticks(ticks, xIsLog, yIsLog,
                   (ga_ticker_t *)0, (ga_labeler_t *)0,
                   (ga_ticker_t *)0, (ga_labeler_t *)0);
}

int ga_draw_alt_ticks(ga_tick_style_t *ticks, int xIsLog, int yIsLog,
              ...
              ga_ticker_t *ytick, ga_labeler_t *ylabel)
{
  ...
  gp_real_t wxmin= gp_transform.window.xmin;
  gp_real_t wxmax= gp_transform.window.xmax;
  gp_real_t wymin= gp_transform.window.ymin;
  gp_real_t wymax= gp_transform.window.ymax;
  ...
  saveMap= gp_transform;
  gp_set_transform(&unitMap);
  gp_clipping= 0;

  if (ticks->horiz.flags & TICK_ANY) {
    altticks= (ticks->horiz.flags & GA_ALT_TICK)? xtick : 0;
    altlabel= (ticks->horiz.flags & GA_ALT_LABEL)? xlabel : 0;
    FindTicks(wxmin, wxmax, &ticks->horiz, xIsLog);

    if (ticks->horiz.flags & GA_TICK_C) {
      /* Axis and ticks to be drawn in middle of viewport */
      gp_real_t yOrigin= FindOrigin(wymin, wymax, &ticks->vert, yIsLog);
      yOrigin= scaly*yOrigin+offy;

      DrawXTicks(yOrigin, ticks->horiz.tickLen, GA_TICK_IN | GA_TICK_OUT, 0,
  ...
  if (ticks->horiz.flags & GA_GRID_F) {
    gp_real_t gridLen[GA_TICK_LEVELS];
    gp_real_t delta= ymax-ymin;
    int i;
    if (nLevel>ticks->horiz.gridLevel)
      nTotal= nChangeLevel[ticks->horiz.gridLevel];
    for (i=0 ; i<=nLevel ; i++) gridLen[i]= delta;
    DrawXTicks(ymin, gridLen, GA_TICK_IN, 0, &ticks->horiz.gridStyle);
  } else if (ticks->horiz.flags & GA_GRID_O) {
    gp_real_t xOrigin= FindOrigin(wxmin, wxmax, &ticks->horiz, xIsLog);
    xOrigin= scalx*xOrigin+offx;
    DrawOrigin(xOrigin, xOrigin, ymin, ymax, &ticks->horiz.gridStyle);
  }

  if (ticks->vert.flags & TICK_ANY) {
    altticks= (ticks->vert.flags & GA_ALT_TICK)? ytick : 0;
    altlabel= (ticks->vert.flags & GA_ALT_LABEL)? ylabel : 0;
    FindTicks(wymin, wymax, &ticks->vert, yIsLog);

    if (ticks->vert.flags & GA_TICK_C) {
      /* Axis and ticks to be drawn in middle of viewport */
      gp_real_t xOrigin= FindOrigin(wxmin, wxmax, &ticks->horiz, xIsLog);
      xOrigin= scalx*xOrigin+offx;

      DrawYTicks(xOrigin, ticks->vert.tickLen, GA_TICK_IN | GA_TICK_OUT, 0,
  ...
  if (ticks->vert.flags & GA_GRID_F) {
    gp_real_t gridLen[GA_TICK_LEVELS];
    gp_real_t delta= xmax-xmin;
    int i;
    if (nLevel>ticks->vert.gridLevel)
      nTotal= nChangeLevel[ticks->vert.gridLevel];
    for (i=0 ; i<=nLevel ; i++) gridLen[i]= delta;
    DrawYTicks(xmin, gridLen, GA_TICK_IN, 0, &ticks->vert.gridStyle);
  } else if (ticks->vert.flags & GA_GRID_O) {
    gp_real_t yOrigin= FindOrigin(wymin, wymax, &ticks->vert, yIsLog);
    yOrigin= scaly*yOrigin+offy;
  ...
  if (ticks->frame) {
    ...
    ga_attributes.l= ticks->frameStyle;
    gp_draw_disjoint(4L, x0, y0, x1, y1);
  }

  gp_set_transform(&saveMap);
  return 0;
}
