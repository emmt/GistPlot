...
static gp_callbacks_t cgm_on = { "gist gp_cgm_engine_t", 0, 0, 0, 0, 0, 0, 0, 0 };
...
#ifndef CREATE_CGM
#define CREATE_CGM(name) pl_fopen(name, "wb")
#endif

/* ------------------------------------------------------------------------ */

static gp_octet_t *FormCommand(gp_octet_t *buffer, int klass, int id, long nbytes,
                          long *lpart);
static gp_octet_t *NextPartition(gp_octet_t *buffer, long nbytes, long *lpart);
static gp_octet_t *Pascalify(gp_octet_t *buffer, const char *text, long len, int pad);
static gp_octet_t *FormWords(gp_octet_t *buffer, const short *words, long nwords);
static void FormReal(short *fixed, gp_real_t x);

static int WriteB(pl_file_t *file, void *buffer, long nbytes);
static void WriteError(gp_cgm_engine_t *cgm, const char *msg);

static int BeginMetafile(gp_cgm_engine_t *cgm);
static void SetPageDefaults(gp_cgm_engine_t *cgmEngine);
static void SetCGMTransform(gp_transform_t *toPixels, int landscape,
                            gp_real_t scale);
static int BeginPage(gp_cgm_engine_t *cgmEngine);
static void EndClip(gp_cgm_engine_t *cgmEngine);
static void BeginClip(gp_cgm_engine_t *cgmEngine, gp_transform_t *trans);
static int EndPage(gp_cgm_engine_t *cgmEngine);
...
static int SetupColor(gp_cgm_engine_t *cgmEngine, unsigned long color, int which);
static int SetupLine(gp_cgm_engine_t *cgmEngine);
static int SetupEdge(gp_cgm_engine_t *cgmEngine);
static void CheckClip(gp_cgm_engine_t *cgmEngine);
static int DrawLines(gp_engine_t *engine, long n, const gp_real_t *px,
                     const gp_real_t *py, int closed, int smooth);
static int SetupMarker(gp_cgm_engine_t *cgmEngine);
static int DrawMarkers(gp_engine_t *engine, long n, const gp_real_t *px,
                       const gp_real_t *py);
static int SetupText(gp_cgm_engine_t *cgmEngine);
...
{
  long nhere;
  *buffer++= (gp_octet_t)((klass<<4) | (id>>3));
  if (nbytes < 0x1f) {
    *buffer++= (gp_octet_t)((id<<5) | nbytes);
    nhere= nbytes;
  } else {
    *buffer++= (gp_octet_t)((id<<5) | (nbytes<0x1f? nbytes : 0x1f));
    if (nbytes < GP_MAX_CGM_PARTITION) {
      nhere= nbytes;
      *buffer++= (gp_octet_t)(nhere>>8);
      *buffer++= (gp_octet_t)(nhere&0xff);
    } else {
      nhere= GP_MAX_CGM_PARTITION;
      *buffer++= (gp_octet_t)((nhere>>8) | 0x80);
      *buffer++= (gp_octet_t)(nhere&0xff);
    }
  }
  *lpart= nhere;
...
{
  while (nwords--) {
    *buffer++= (gp_octet_t)(*words >> 8);
    *buffer++= (gp_octet_t)(*words++ & 0xff);
  }
  return buffer;
}
...
{
  if (!file) return 1;
  return pl_fwrite(file, buffer, nbytes) != (unsigned long)nbytes;
}
...
static int BeginMetafile(gp_cgm_engine_t *cgm)
{
  ...
  char description[88];
  gp_octet_t *descriptor, *now, *font;
  short value[3];
  ...
  strncat(description, pl_getuser(), 50L);
  ...
  len+= 4 + lfonts;
  if (len&1) len++;
  descriptor= (gp_octet_t *)pl_malloc(len+2);
  if (!descriptor) {
    strcpy(gp_error, "memory manager failed in BeginMetafile");
  ...
  if (!file) {
    strcpy(gp_error, "unable to create CGM output");
    cgm->state= 1;     /* Metafile closed */
    pl_free(descriptor);
    return 1;
  }
  ...
  *now++= 2;   /* begin at color index 2 (PL_BLACK, which is -3) */
...
static void SetPageDefaults(gp_cgm_engine_t *cgmEngine)
{
  ...
  for (i=0 ; i<5 ; i++)
    cgmEngine->curColor[i]= PL_FG; /* CGM color index #1 */
  cgmEngine->curType= GP_LINE_SOLID;
  cgmEngine->curWidth= 1.0;
  cgmEngine->curMark= GP_MARKER_ASTERISK;
  cgmEngine->curSize= 1.0;
  cgmEngine->curFont= GP_FONT_TIMES;   /* CGM font #1 */
  cgmEngine->curHeight= 0.0;     /* Default is unusable (1 VDC) */
  cgmEngine->curAlignH= GP_HORIZ_ALIGN_NORMAL;
  cgmEngine->curAlignV= GP_VERT_ALIGN_NORMAL;
  cgmEngine->curPath= GP_ORIENT_RIGHT;
  cgmEngine->curOpaque= 0;
  cgmEngine->curEtype= GP_LINE_NONE;
...
static int BeginPage(gp_cgm_engine_t *cgmEngine)
{
  pl_file_t *file= cgmEngine->file;
  long len, lpage;
  gp_octet_t command[44], *now;
  char page[30];
  short xy[4];
  ...
  ChangePalette((gp_engine_t *)cgmEngine);
  ...
  if (cgmEngine->e.colorMode && cgmEngine->e.palette &&
    ...
    pl_col_t *palette= cgmEngine->e.palette;
    gp_octet_t *col;
    if (nColors>246) nColors= 246;

    col= (gp_octet_t *)pl_malloc(3*nColors+6);
    if (!col) {
    ...
    *now++= 10;   /* begin at color index 10 */
    for (i=0 ; i<nColors ; i++) {
      *now++= (gp_octet_t)(PL_R(palette[i]));
      *now++= (gp_octet_t)(PL_G(palette[i]));
      *now++= (gp_octet_t)(PL_B(palette[i]));
    }
    if (!(nColors&1)) *now++= 0;
...
static void BeginClip(gp_cgm_engine_t *cgmEngine, gp_transform_t *trans)
{
  ...
  gp_box_t *box= &cgmEngine->clipBox;
  gp_octet_t command[16], *now;
  pl_file_t *file;
  long len;
  ...
  if (port->xmin!=box->xmin || port->xmax!=box->xmax ||
    ...
    gp_int_points(&cgmEngine->e.map, 3, 2, x, y, &points);
    if (points[0].x > points[1].x) {
      gp_real_t xt= points[0].x;
      points[0].x= points[1].x;
      points[1].x= (short)xt;
    }
    if (points[0].y > points[1].y) {
      gp_real_t yt= points[0].y;
      points[0].y= points[1].y;
      points[1].y= (short)yt;
    }

    now= FormCommand(now, 3, 5, 8L, &len);           /* CLIP RECTANGLE */
    /* Note-- this cast from gp_point_t* to short* works on all machines
...
static int EndPage(gp_cgm_engine_t *cgmEngine)
{
  ...
  /* Write END PICTURE END METAFILE, then backspace over
     END METAFILE.  Combined with the pl_fflush, this assures
     that a legal metafile is present on disk after EndPage.  */
  oops = WriteB(file, command, 4L);
  if (!oops) pos = pl_ftell(file), oops = (pos==-1L || pl_fseek(file, pos-2));
  if (oops) {
    WriteError(cgmEngine, "write to CGM failed in EndPage");
    return 1;
  }

  if (pl_ftell(file) < (unsigned long)cgmEngine->fileSize) {
...
static int ChangePalette(gp_engine_t *engine)
{
  gp_cgm_engine_t *cgmEngine= (gp_cgm_engine_t *)engine;
...
{
  gp_cgm_engine_t *cgmEngine= (gp_cgm_engine_t *)engine;
  if (cgmEngine->file) pl_fflush(cgmEngine->file);
  return 0;
}
...
static int SetupColor(gp_cgm_engine_t *cgmEngine, unsigned long color, int which)
{
  ...
  if (color<240UL) {
    ...
    if (color<PL_YELLOW) color= PL_FG;
...
static int SetupLine(gp_cgm_engine_t *cgmEngine)
{
  pl_file_t *file= cgmEngine->file;
  long len;
  gp_octet_t command[14], *now;

  if (cgmEngine->state!=4) return 1;
  if (SetupColor(cgmEngine, ga_attributes.l.color, 0)) return 1;
  now= command;

  if (cgmEngine->curType!=ga_attributes.l.type) {
    short ltype= (short)ga_attributes.l.type;
    if (ltype==GP_LINE_NONE) return 1;

    if (ltype<=0 || ltype>GP_LINE_DASHDOTDOT) ltype= GP_LINE_SOLID;

    now= FormCommand(now, 5, 2, 2L, &len);                  /* LINE TYPE */
    now= FormWords(now, &ltype, 1L);

    cgmEngine->curType= ga_attributes.l.type;
  }

  if (cgmEngine->curWidth!=ga_attributes.l.width) {
    short lwidth[2];

    now= FormCommand(now, 5, 3, 4L, &len);                 /* LINE WIDTH */
    FormReal(lwidth, ga_attributes.l.width);
    now= FormWords(now, lwidth, 2L);

    cgmEngine->curWidth= ga_attributes.l.width;
  }

  if (cgmEngine->curOpaque!=0 && cgmEngine->curType!=GP_LINE_SOLID) {
    short trans= 0;
...
static int SetupEdge(gp_cgm_engine_t *cgmEngine)
{
  ...
  if (cgmEngine->state!=4) return 1;
  if (SetupColor(cgmEngine, ga_attributes.e.color, 4)) return 1;
  now= command;

  if (cgmEngine->curEtype!=ga_attributes.e.type) {
    short ltype= (short)ga_attributes.e.type;
    short visible= (ltype!=GP_LINE_NONE);
    int change= visible^(cgmEngine->curEtype!=GP_LINE_NONE);
    ...
    cgmEngine->curEtype= ga_attributes.e.type;
  }

  if (cgmEngine->curEwidth!=ga_attributes.e.width) {
    short lwidth[2];

    now= FormCommand(now, 5, 28, 4L, &len);                /* EDGE WIDTH */
    FormReal(lwidth, ga_attributes.e.width);
    now= FormWords(now, lwidth, 2L);

    cgmEngine->curEwidth= ga_attributes.e.width;
  }

  if (cgmEngine->curOpaque!=0 && cgmEngine->curEtype!=GP_LINE_SOLID) {
    short trans= 0;
...
static void CheckClip(gp_cgm_engine_t *cgmEngine)
{
  ...
  if (gp_clipping) BeginClip(cgmEngine, &gp_transform);
                     ...
                     const gp_real_t *py, int closed, int smooth)
{
  ...
  while ((nPoints=
    ...
    /* The cast from gp_point_t* to short* works on all machines I know of,
    ...
    if (WriteB(file, (gp_octet_t *)points, nPoints<<2)) {
      WriteError(cgmEngine, "write to CGM failed in DrawLines");
      return 1;
    }
...
static int SetupMarker(gp_cgm_engine_t *cgmEngine)
{
  ...
  if (cgmEngine->state!=4) return 1;
  if (SetupColor(cgmEngine, ga_attributes.m.color, 1)) return 1;
  now= command;

  if (cgmEngine->curMark!=ga_attributes.m.type) {
    short mtype= (short)ga_attributes.m.type;

    now= FormCommand(now, 5, 6, 2L, &len);                /* MARKER TYPE */
    now= FormWords(now, &mtype, 1L);

    cgmEngine->curMark= ga_attributes.m.type;
  }

  if (cgmEngine->curSize!=ga_attributes.m.size) {
    short msize[2];

    now= FormCommand(now, 5, 7, 4L, &len);                /* MARKER SIZE */
    FormReal(msize, ga_attributes.m.size);
    now= FormWords(now, msize, 2L);

    cgmEngine->curSize= ga_attributes.m.size;
  }
...
static int SetupText(gp_cgm_engine_t *cgmEngine)
{
  ...
  int opq;
  long len;
  gp_octet_t command[36], *now;
  int h= ga_attributes.t.alignH, v= ga_attributes.t.alignV;
  gp_get_text_alignment(&ga_attributes.t, &h, &v);

  if (cgmEngine->state!=4) return 1;
  if (SetupColor(cgmEngine, ga_attributes.t.color, 2)) return 1;
  now= command;

  if (cgmEngine->curFont!=ga_attributes.t.font) {
    int fn= (ga_attributes.t.font>=0 && ga_attributes.t.font<N_CGMFONTS)?
      ga_attributes.t.font : 0;
    short font= (short)cgmFontNumbers[fn];

    now= FormCommand(now, 5, 10, 2L, &len);           /* TEXT FONT INDEX */
    now= FormWords(now, &font, 1L);

    cgmEngine->curFont= ga_attributes.t.font;
  }

  if (cgmEngine->curHeight!=ga_attributes.t.height) {
    /* -----------------------WARNING----------------------------- */
    ...
    /* -----------------------WARNING----------------------------- */
    short ch= (short)(0.8*ga_attributes.t.height*cgmEngine->scale+0.5);

    now= FormCommand(now, 5, 15, 2L, &len);          /* CHARACTER HEIGHT */
    now= FormWords(now, &ch, 1L);

    cgmEngine->curHeight= ga_attributes.t.height;
  }
  ...
  if (cgmEngine->curPath!=ga_attributes.t.orient) {
    short orient[4];
    ...
    cgmEngine->curPath= ga_attributes.t.orient;
  }
...
static gp_octet_t cgmText[264];  /* header + point + flag + 254 pascal chars */
...
static int DrwText(gp_engine_t *engine, gp_real_t x0, gp_real_t y0, const char *text)
{
  ...
  if ((short)(0.8*ga_attributes.t.height*cgmEngine->scale+0.5) <= 0) return 0;
  ...
  lineHeight= ga_attributes.t.height * cgmEngine->scale;
  ...
  while ((t= gp_next_text_line(text, &count, GP_ORIENT_RIGHT))) {
    if (count>0) {
    ...
    text= t+count;
    if (ga_attributes.t.orient==GP_ORIENT_RIGHT) y0-= lineHeight;
    else if (ga_attributes.t.orient==GP_ORIENT_UP) x0+= lineHeight;
    else if (ga_attributes.t.orient==GP_ORIENT_LEFT) y0+= lineHeight;
    else x0-= lineHeight;
  }
                    ...
                    const gp_real_t *py)
{
  ...
  if (n<3) return 0;
  CheckClip(cgmEngine);
  if (SetupColor(cgmEngine, ga_attributes.f.color, 3) ||
                     ...
                     const gp_color_t *colors)
{
  gp_cgm_engine_t *cgmEngine= (gp_cgm_engine_t *)engine;
  pl_file_t *file;
  gp_xymap_t *map= &cgmEngine->e.map;
  int nColors= cgmEngine->nColors;
  pl_col_t *palette;
  short pqrwhd[10];
  long i, j, off, nCells, lPart;
  int pad, color, colorMode;
  gp_octet_t *buffer, *now;
  ...
  lPart= nCells+20;
  if (lPart>GP_MAX_CGM_PARTITION) lPart= GP_MAX_CGM_PARTITION;
  if (lPart & 1) lPart++;
  buffer= (gp_octet_t *)pl_malloc(lPart+4);
  if (!buffer) {
                        ...
                        const gp_real_t *py, const gp_real_t *qx, const gp_real_t *qy)
{
  ...
  while ((nSegs=
    ...
    /* The cast from gp_segment_t* to short* works on all machines I know of,
       but may fail someday...  Note that maxSegs is set low enough
       that multiple partitions will never be required. */
    CGM_WORD_ORDER((short *)segs, nSegs<<2);
    if (WriteB(file, (gp_octet_t *)segs, nSegs<<3)) {
      WriteError(cgmEngine, "write to CGM failed in DrawDisjoint");
      return 1;
    }
...
gp_real_t gp_cgm_scale= 25545.24;   /* default CGM scale is 2400 dpi (round up) */
long gp_cgm_file_size= 100000000; /* default max file size is about 100 Meg */

gp_engine_t *gp_new_cgm_engine(char *name, int landscape, int mode, char *file)
{
  gp_cgm_engine_t *cgmEngine;
  long flen= file? strlen(file) : 0;
  long engineSize= sizeof(gp_cgm_engine_t)+flen+1;
  gp_transform_t toPixels;

  if (flen<=0) return 0;

  SetCGMTransform(&toPixels, landscape, gp_cgm_scale);

  cgmEngine=
    (gp_cgm_engine_t *)gp_new_engine(engineSize, name, &cgm_on, &toPixels, landscape,
  ...
  if (!cgmEngine) {
    strcpy(gp_error, "memory manager failed in gp_new_cgm_engine");
    return 0;
  }

  cgmEngine->filename= (char *)(cgmEngine+1);
  strcpy(cgmEngine->filename, file);
  cgmEngine->scale= gp_cgm_scale;
  cgmEngine->fileSize= gp_cgm_file_size;
  ...
  return (gp_engine_t *)cgmEngine;
}

gp_cgm_engine_t *gp_cgm_engine(gp_engine_t *engine)
{
  return (engine && engine->on==&cgm_on)? (gp_cgm_engine_t *)engine : 0;
}

void gp_cgm_set_scale(gp_cgm_engine_t *cgmEngine, gp_real_t scale)
{
  gp_transform_t toPixels;
