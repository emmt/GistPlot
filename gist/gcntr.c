 ...
 * explained in gist2.h in the definition of the ga_mesh_t struct.  */
...
struct Csite {
  ...
  gp_real_t zlevel[2];    /* contour levels, zlevel[1]<=zlevel[0]
  ...
  const gp_real_t *x, *y, *z;   /* mesh coordinates and function values */
  gp_real_t *xcp, *ycp;         /* output contour points */
};
...
static int zone_crosser(Csite *site, Cdata *data, int level, int pass2)
{
  ...
  const gp_real_t *x= pass2? site->x : 0;
  const gp_real_t *y= pass2? site->y : 0;
  const gp_real_t *z= pass2? site->z : 0;
  gp_real_t zlevel= pass2? site->zlevel[level] : 0.0;
  gp_real_t *xcp= pass2? site->xcp : 0;
  gp_real_t *ycp= pass2? site->ycp : 0;
  ...
  for (;;) {
    ...
    if (pass2) {
      ...
      gp_real_t zcp= (zlevel-z[p0])/(z[p1]-z[p0]);
...
static int edge_walker(Csite *site, Cdata *data, int pass2)
{
  ...
  for (;;) {
    ...
    if (z0==1) {
      ...
      if (pass2) {
        gp_real_t zcp= site->zlevel[(z0!=0)];
...
static void data_init(Csite *site, Cdata *data, int region, long nchunk)
{
  ...
  const gp_real_t *z= site->z;
  gp_real_t zlev0= site->zlevel[0];
  gp_real_t zlev1= site->zlevel[1];
                   ...
                   gp_real_t levs[2], long nchunk, long *nparts)
{
  ...
  *nparts= 0;
  ...
  if (gc_site.zlevel[0] > gc_site.zlevel[1]) {
    gp_real_t tmp= gc_site.zlevel[0];
...
long gp_cont_trace(long *n, gp_real_t *px, gp_real_t *py)
{
  long np, ntotal= 0;
