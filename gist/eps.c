...
#define PS_TO_NDC (GP_ONE_POINT/20.0)

static gp_callbacks_t g_eps_on = { "gist EPS pseudo-engine",
                                0, 0, 0, 0, 0, 0, 0, 0 };

static pl_file_t *psFile= 0, *epsFile= 0;
static unsigned char *epsPreview= 0;
static char line[256];
static int xll, xur, yll, yur, epsLandscape;
static gp_real_t epsXScale, epsYScale;
...
static void Rasterize(gp_point_t *p0, gp_point_t *p1);
static void ScaleFont(gp_real_t ndcHeight);
...
/* FrameMaker 3.0X has a bug in its EPS preview display to X Windows,
   ...
   correctly).  Set gp_eps_fm_bug to compensate for this.  */
int gp_eps_fm_bug= 0;
...
static void Kill(gp_engine_t *engine)
{
  ...
  if (!pl_fgets(psFile, line, 256)) goto err;
  pl_fputs(epsFile, "%!PS-Adobe-3.0 EPSF-3.0\n");
  ...
  do {
    ...
    pl_fputs(epsFile, line);
  } while (strncmp("%%EndComments", line, 13L));

  /* Write preview comments */
  pl_fputs(epsFile, "%%BeginPreview: 256 256 1 256\n");
  line[0]= '%';
  line[1]= ' ';
  line[66]= '\n';
  line[67]= '\0';
  ...
  pl_fputs(epsFile, "%%EndPreview\n");

  /* Copy rest of PS file, omitting bounding box at end */
  for (;;) {
    if (!pl_fgets(psFile, line, 256)) break;
    if (strncmp("%%BoundingBox:", line, 14L)) pl_fputs(epsFile, line);
  }
...
static gp_real_t xAlpha[4]= { 0.05, 0.30, 0.55, 0.60 };
static gp_real_t yAlpha[7]= { -0.18, 0.00, 0.18, 0.36, 0.54, 0.72, 0.80 };
static gp_real_t xChar[4], yChar[7], xCurrent, yCurrent;
static gp_xymap_t *charMap;

static void ScaleFont(gp_real_t ndcHeight)
{
  gp_real_t yScale= ndcHeight*epsYScale;
  gp_real_t xScale= ndcHeight*epsXScale;
  int i;
                     ...
                     const gp_real_t *py, int closed, int smooth)
{
  ...
  if (n<1 || ga_attributes.l.type==GP_LINE_NONE) return 0;
...
static int DrwText(gp_engine_t *engine, gp_real_t x0, gp_real_t y0, const char *text)
{
  ...
  int alignH= ga_attributes.t.alignH, alignV= ga_attributes.t.alignV;
  int count;
  const char *t;

  charMap= &engine->map;
  ScaleFont(ga_attributes.t.height);
  ...
  if (ga_attributes.t.orient==GP_ORIENT_RIGHT) {
    if (alignV==GP_VERT_ALIGN_TOP) y0-= yChar[6];
    else if (alignV==GP_VERT_ALIGN_CAP) y0-= yChar[5];
    else if (alignV==GP_VERT_ALIGN_HALF) y0-= yChar[3];
    else if (alignV==GP_VERT_ALIGN_BOTTOM) y0-= yChar[0];
  } else if (ga_attributes.t.orient==GP_ORIENT_LEFT) {
    if (alignV==GP_VERT_ALIGN_TOP) y0+= yChar[6];
    else if (alignV==GP_VERT_ALIGN_CAP) y0+= yChar[5];
    else if (alignV==GP_VERT_ALIGN_HALF) y0+= yChar[3];
    ...
    if (alignV==GP_VERT_ALIGN_TOP) x0+= yChar[6];
    else if (alignV==GP_VERT_ALIGN_CAP) x0+= yChar[5];
    else if (alignV==GP_VERT_ALIGN_HALF) x0+= yChar[3];
    else if (alignV==GP_VERT_ALIGN_BOTTOM) x0+= yChar[0];
  } else {
    if (alignV==GP_VERT_ALIGN_TOP) x0-= yChar[6];
    else if (alignV==GP_VERT_ALIGN_CAP) x0-= yChar[5];
    else if (alignV==GP_VERT_ALIGN_HALF) x0-= yChar[3];
  ...
  while ((t= gp_next_text_line(text, &count, ga_attributes.t.orient))) {
    /* Set xCurrent to left edge of current line */
    if (ga_attributes.t.orient==GP_ORIENT_RIGHT) {
      if (alignH==GP_HORIZ_ALIGN_CENTER) xCurrent= x0 - 0.5*width*(gp_real_t)count;
      else if (alignH==GP_HORIZ_ALIGN_RIGHT) xCurrent= x0 - width*(gp_real_t)count;
      else xCurrent= x0;
    } else if (ga_attributes.t.orient==GP_ORIENT_UP) {
      if (alignH==GP_HORIZ_ALIGN_CENTER) yCurrent= y0 + 0.5*width*(gp_real_t)count;
      else if (alignH==GP_HORIZ_ALIGN_RIGHT) yCurrent= y0 + width*(gp_real_t)count;
      else yCurrent= y0;
    } else if (ga_attributes.t.orient==GP_ORIENT_LEFT) {
      if (alignH==GP_HORIZ_ALIGN_CENTER) xCurrent= x0 + 0.5*width*(gp_real_t)count;
      else if (alignH==GP_HORIZ_ALIGN_RIGHT) xCurrent= x0 + width*(gp_real_t)count;
    ...
    RenderChars(t, count, ga_attributes.t.orient);
    text= t+count;

    if (ga_attributes.t.orient==GP_ORIENT_RIGHT) yCurrent-= lineHeight;
    else if (ga_attributes.t.orient==GP_ORIENT_UP) xCurrent+= lineHeight;
    else if (ga_attributes.t.orient==GP_ORIENT_LEFT) yCurrent+= lineHeight;
                        ...
                        const gp_real_t *py, const gp_real_t *qx, const gp_real_t *qy)
{
  ...
  while ((nSegs=
    ...
    /* This cast from gp_segment_t to gp_point_t is lazy programming,
       but I don't know of any platforms it will not work on... */
    points= (gp_point_t *)segs;
...
gp_engine_t *gp_eps_preview(gp_engine_t *engine, char *file)
{
  gp_ps_engine_t *psEngine= (gp_ps_engine_t *)engine;
  gp_transform_t toPixels;
  int i;

  /* Grab bounding box from the PS gp_engine_t */
  ...
  toPixels.viewport.xmin= (gp_real_t)xll*PS_TO_NDC;
  toPixels.viewport.xmax= (gp_real_t)xur*PS_TO_NDC;
  toPixels.viewport.ymin= (gp_real_t)yll*PS_TO_NDC;
  toPixels.viewport.ymax= (gp_real_t)yur*PS_TO_NDC;
  ...
  epsFile= pl_fopen(file, "w");
  if (!epsFile) return 0;
  epsPreview= (unsigned char *)pl_malloc(sizeof(char)*32*256);
  ...
  /* Create the EPS gp_engine_t */
  engine= gp_new_engine(sizeof(gp_engine_t), "Gist EPS", &g_eps_on, &toPixels, 0,
                      &Kill, &Clear, &Flush, &gp_compose_map,
                      &ChangePalette, &DrawLines, &gp_draw_pseudo_mark,
